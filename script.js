document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('click', (e) => {
        if (e.target == modalNovaCarga || e.target == modalDetalhes) fecharModais();
    });
    ...
    function renderizarModalDetalhes() {
        ...
        } else { // Fallback para status finalizada ou outros
            secaoDados = `<div class="detalhes-secao"><h4>Dados da Viagem</h4><div class="detalhes-form-grid-4"><div class="campo-form"><label>Origem</label><p>${detalhes_carga.origem || ''}</p></div><div class="campo-form"><label>Peso Total</label><p>${formatarPeso(pesoTotal)}</p></div><div class="campo-form"><label>Frete Total</label><p>${formatarMoeda(freteTotal)}</p></div><div class="campo-form"><label>Qtd. Entregas</label><p>${entregas.length}</p></div></div></div>`;
            secaoAcoes = '';
        }
        ...
        detalhesConteudo.innerHTML = `
            ...
            <div class="modal-body-grid">
                ${secaoDados}
                <div class="detalhes-secao secao-full-width">
                    <h4>Observações da Viagem</h4>
                    <div class="form-acao"><textarea id="obs-carga" rows="4" ${!podeEditarGeral ? 'disabled' : ''}>${detalhes_carga.observacoes || ''}</textarea></div>
                </div>
                ${secaoAcoes}
            </div>
        ...
        const tabelaCorpo = document.getElementById('tabela-entregas-corpo');
        tabelaCorpo.innerHTML = entregas.length > 0 ? entregas.map(e => `<tr><td>${e.razao_social}</td><td>${e.cidade}-${e.estado}</td><td>${formatarPeso(e.peso_bruto)}</td><td>(${e.ddd||''}) ${e.telefone||''}</td><td title="${e.obs_cliente || ''}">${(e.obs_cliente || 'Nenhuma').substring(0, 20)}</td>${podeEditarEntregas ? `<td><button class="btn-excluir-entrega" data-id="${e.id}">Excluir</button></td>` : ''}</tr>`).join('') : `<tr><td colspan="${podeEditarEntregas ? 6:5}">Nenhuma entrega.</td></tr>`;
    ...
    async function handleFinalizarCarga() {
        ...
        if (response.ok) {
            ...
            enviarAtualizacaoStatus({ status: 'Finalizada', data_finalizacao: hoje });
    ...
    document.getElementById('form-nova-carga').addEventListener('submit', async (e) => {
        ...
        const response = await fetch('/api/cargas', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ origem: document.getElementById('origem').value }),
        });
        if (response.ok) {
            fecharModais();
            carregarDadosIniciais();
        } else { alert('Erro ao criar a carga.'); }
    });
