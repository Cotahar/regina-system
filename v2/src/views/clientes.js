export function renderClientesPage() {
  return `
    ...
    <div class="card mt-4">
      <h2 class="mb-3 text-sm font-semibold text-slate-700">Importar planilha (CSV: codigo, razao social, ddd, telefone, cidade, estado, observacoes)</h2>
      ...
      <details class="mt-3">
        <summary class="cursor-pointer text-xs text-slate-600">ou colar linhas copiadas do Excel (mesma ordem de colunas, sem cabecalho)</summary>
    ...
    <div id="modal-cliente" class="fixed inset-0 z-20 hidden items-center justify-center overflow-y-auto bg-slate-900/50 py-8">
      <form id="form-cliente" class="card w-full max-w-xl">
        <h2 class="mb-4 text-lg font-semibold" id="modal-titulo">Novo cliente</h2>
        <input type="hidden" id="cliente-id">

        <div class="mb-3">
          <label class="mb-1 block text-xs text-slate-600">Codigo do cliente</label>
          <input type="text" id="cliente-codigo" class="input-field" placeholder="Deixe em branco para gerar automaticamente">
        </div>

        <div class="mb-3">
          <label class="mb-1 block text-xs text-slate-600">Razao social</label>
          <input type="text" id="cliente-razao" class="input-field" required>
        </div>

        <div class="mb-3 grid grid-cols-2 gap-3">
          <div>
            <label class="mb-1 block text-xs text-slate-600">Cidade</label>
            <input type="text" id="cliente-cidade" class="input-field">
          </div>
          <div>
            <label class="mb-1 block text-xs text-slate-600">Estado (UF)</label>
        ...
        <div class="mb-3 grid grid-cols-2 gap-3">
          <div>
            <label class="mb-1 block text-xs text-slate-600">DDD</label>
            <input type="text" id="cliente-ddd" class="input-field" maxlength="2">
          </div>
          <div>
            <label class="mb-1 block text-xs text-slate-600">Telefone</label>
            <input type="text" id="cliente-telefone" class="input-field">
          </div>
        </div>

        <div class="mb-3">
          <label class="mb-1 block text-xs text-slate-600">Observacoes</label>
        ...
        <div class="mb-3">
          <label class="mb-1 block text-xs text-slate-600">Contato adicional (se houver mais de um)</label>
          <input type="text" id="cliente-contato-extra" class="input-field" placeholder="Nome / telefone do segundo contato">
        </div>

        <div class="mb-4">
          <p class="mb-2 text-xs text-slate-600">Perfil de atendimento (informacoes para quem for buscar/entregar)</p>
        ...
        <div class="mb-4 grid grid-cols-2 gap-3">
          <div>
            <label class="mb-1 block text-xs text-slate-600">Forma de pagamento padrao</label>
            <select id="cliente-forma-pagamento" class="input-field">
              <option value="">-</option>
            </select>
          </div>
          <div>
            <label class="mb-1 block text-xs text-slate-600">Tipo de pagamento padrao</label>
