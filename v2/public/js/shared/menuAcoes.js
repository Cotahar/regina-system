...
export function criarMenuAcoes(trigger, itens) {
  ...
  menu.className = 'absolute right-0 z-40 mt-1 hidden min-w-[9rem] rounded-md border border-painel-border bg-slate-900 py-1 shadow-lg';
  menu.innerHTML = itens.map((item, i) => `
    <button type="button" data-i="${i}" class="block w-full px-3 py-1.5 text-left text-xs ${item.perigo ? 'text-red-400 hover:bg-red-500/10' : 'text-slate-200 hover:bg-painel-border'}">${item.label}</button>
  `).join('');
