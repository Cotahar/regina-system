...
export function iconeMenuAcoes(dataAttrs = '') {
  return `
    <button type="button" class="btn-menu-acoes inline-flex h-7 w-7 items-center justify-center rounded-md text-slate-600 hover:bg-painel-border hover:text-slate-900" ${dataAttrs}>
...
export function criarMenuAcoes(trigger, itens) {
  ...
  menu.className = 'absolute right-0 z-40 mt-1 hidden min-w-[9rem] rounded-md border border-painel-border bg-white py-1 shadow-lg';
  menu.innerHTML = itens.map((item, i) => `
    <button type="button" data-i="${i}" class="block w-full px-3 py-1.5 text-left text-xs ${item.perigo ? 'text-red-600 hover:bg-red-50' : 'text-slate-800 hover:bg-gray-50'}">${item.label}</button>
  `).join('');
